      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
        ...
        {progress && (
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: t('totalPoints'),   value: progress.totalPoints,   icon: '⭐', color: 'text-amber-600' },
              { label: t('currentStreak'), value: `${progress.currentStreak} ${t('days')}`, icon: '🔥', color: 'text-red-500' },
              { label: t('completedLessons'), value: `${progress.completedLessons}/${progress.totalLessons}`, icon: '✅', color: 'text-teal-600' },
              { label: t('changeLevel'),   value: progress.currentLevel,  icon: '📚', color: 'text-blue-600' },
            ].map((s) => (
        ...
        <div className="bg-white rounded-3xl border border-gray-100 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-50">
            <p className="font-black text-gray-800 text-sm uppercase tracking-wide">{t('settings')}</p>
          </div>

          {/* Change language */}
          <div className="px-5 py-4 border-b border-gray-50">
            <div className="flex justify-between items-center mb-2">
              <p className="font-bold text-gray-700 text-sm">{t('changeLanguage')}</p>
              <button
                onClick={() => { setEditLang(!editLang); setSelLang(nativeLanguage as SupportedLanguage) }}
            ...
            {!editLang ? (
              ...
              <div className="grid grid-cols-3 gap-2 mt-3">
                {LANGUAGES.map((lang) => (
                  <button
                    ...
                    className={`p-2 rounded-xl border text-center text-xs font-bold transition-all ${
                      selLang === lang.code ? 'border-red-400 bg-red-50' : 'border-gray-100'
                    }`}
                  >
                    {lang.flag} {lang.name}
                  </button>
                ...
                <button
                  onClick={() => updateMutation.mutate({ nativeLanguage: selLang, level: selLevel || 1 })}
          ...
          <div className="px-5 py-4">
            <div className="flex justify-between items-center mb-2">
              <p className="font-bold text-gray-700 text-sm">{t('changeLevel')}</p>
              <button
            ...
            {!editLevel ? (
              <p className="text-gray-500 text-sm">📚 {progress?.currentLevel}</p>
            ) : (
              <div className="flex flex-col gap-2 mt-3">
                {LEVELS.map((lv) => (
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left text-sm font-bold transition-all ${
                      selLevel === lv.value ? 'border-red-400 bg-red-50' : 'border-gray-100'
                    }`}
                  >
                    {lv.emoji} {lv.label}
                  </button>
                ...
                <button
                  onClick={() => updateMutation.mutate({ nativeLanguage: selLang || nativeLanguage, level: selLevel })}
